...
let targets = [...store.keys()]
console.log(targets)

mockData.children = targets.map(target => {
  let targetData = {
    id: uuidv4(),
  ...
  let bucket = store.get(target)
  console.log(bucket)

  let keys = [...bucket.keys()]
  let targetChildren = keys.map(key => {
    let keyData = {
      id: uuidv4(),
      name: key,
      rate: 0
    ...
    let keyChildren = effects.map(effect => {
      return {
        id: uuidv4(),
        name: effect.fn.name,
        rate: 0
    ...
    keyData.children = keyChildren
    return keyData
  })

  targetData.children = targetChildren
